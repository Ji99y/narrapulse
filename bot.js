...
const SIGNALS_API = process.env.SIGNALS_API_URL || 'https://narrapulse.vercel.app/api/signals';
...
function detectRegime(metrics) {
  ...
  if (btcDom > 58)    return { regime: 'BTC_DOMINANCE', risk: 'LOW',  note: 'Altcoins suppressed — be very selective' };
  if (btcDom < 50)    return { regime: 'ALTSEASON',     risk: 'HIGH', note: 'Broad altcoin momentum — wider opportunities' };
  if (volMcap > 0.05) return { regime: 'ACTIVE',        risk: 'MED',  note: 'Healthy volume — standard position sizing' };
  return               { regime: 'QUIET',               risk: 'LOW',  note: 'Low volume — reduce position sizes' };
}
...
function getDivergenceSignal(q) {
  ...
  if (vt.trend==='SURGING'||vt.trend==='RISING') return { signal: 'CONFIRMED' };
...
function scoreToken(token, btc24h = 0) {
  ...
  return {
    symbol: token.symbol, id: token.id, price: q.price,
    c1, c24, c7, volTrend, divergence, narAge, pumpStatus,
...
async function runCheck() {
  ...
  try {
    ...
    const maxPos   = fg.gated ? 1 : (regime.risk === 'HIGH' ? 5 : regime.risk === 'MED' ? 3 : 2);
    const allocPct = fg.gated ? 10 : (regime.risk === 'HIGH' ? 10 : regime.risk === 'MED' ? 15 : 20);

    const btcToken = tokens.find(t => t.symbol === 'BTC');
    const btc24h   = btcToken?.quote?.USD?.percent_change_24h || 0;

    const scored = tokens.map(t => scoreToken(t, btc24h)).filter(Boolean).sort((a, b) => b.score - a.score);
    ...
    top10.forEach(t => { t.narrative = classifyNarrative(catData[t.id]?.tags || []); });
    ...
    const hotNarrative = Object.entries(narrativeMap)
      .map(([name, d]) => ({ name, avg: d.total / d.syms.length }))
      .sort((a, b) => b.avg - a.avg)[0]?.name || 'Unknown';

    const watchlist = top10.filter(t =>
      t.allPos &&
    ...
    const closedSymbols = lastSymbols.filter(s => !currentSymbols.includes(s));
    ...
    for (const t of newSignals) {
      ...
      await postSignalOpen({
        ...
        target:       parseFloat((t.price * 1.20).toFixed(6)),
        score:        t.score,
    ...
    saveLastSignals(watchlist.map(t => ({
      symbol:    t.symbol,
      entry:     t.price,
      stop:      parseFloat((t.price * 0.93).toFixed(6)),
      target:    parseFloat((t.price * 1.20).toFixed(6)),
...
runCheck();
console.log('NarraPulse bot started — running every 4 hours');